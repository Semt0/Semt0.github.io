...
!!! abstract "定义 1（级数收敛）"
    ...
    则称复数项级数 **收敛** 于 $s$，$s$ 称为级数的 **和** 。若 $\{s_n\}$ 没有极限，则称级数 **发散** 。
...
!!! abstract "定理 1（收敛的充要条件）"
    级数 $\displaystyle\sum_{n=1}^{\infty} w_n = \sum_{n=1}^{\infty} (u_n + iv_n)$ **收敛的充要条件** 是实数项级数 $\displaystyle\sum_{n=1}^{\infty} u_n$ 和 $\displaystyle\sum_{n=1}^{\infty} v_n$ 都收敛。

??? note "说明"
    复数项级数的审敛问题 $\Leftrightarrow$ 实数项级数的审敛问题

    根据复数列极限存在的充要条件：$\{s_n\}$ 极限存在 $\Leftrightarrow$ $\{\sigma_n\}$ 和 $\{\tau_n\}$（实部和虚部）的极限都存在。

### （三）柯西判据与绝对收敛

!!! abstract "定理 2（柯西判据）"
    级数 $\displaystyle\sum_{n=1}^{\infty} w_n$ 收敛的充要条件是：对于任意小的正数 $\varepsilon$，存在自然数 $N$，使得当 $n > N$ 时，对任意正整数 $p$ 有

    $$
    |w_{n+1} + w_{n+2} + \cdots + w_{n+p}| < \varepsilon
    $$

!!! abstract "定义 2（绝对收敛）"
    若级数 $\displaystyle\sum_{n=1}^{\infty} |w_n| = \sum_{n=1}^{\infty} \sqrt{u_n^2 + v_n^2}$ 收敛，则称原级数 $\displaystyle\sum_{n=1}^{\infty} w_n$ **绝对收敛** 。
...
!!! abstract "定义 3（一致收敛）"
    任给 $\varepsilon > 0$，存在一个与 $z$ 无关的自然数 $N(\varepsilon)$，当 $n > N(\varepsilon)$ 时，对 $B$（或 $l$）上 **所有** $z$，均有

    $$
    |w_{n+1}(z) + w_{n+2}(z) + \cdots + w_{n+p}(z)| < \varepsilon \quad \text{（$p$ 为任意自然数）}
    $$

    则称级数在 $B$（或 $l$）上 **一致收敛** 。

!!! abstract "定理 3（一致收敛的性质）"
    **性质 1（连续性）**：若 $w_k(z)$ 在 $B$ 内连续，函数级数在 $B$ 内一致收敛，则和函数 $w(z)$ 也是 $B$ 内的连续函数。

    $$
    \lim_{z \to z_0} \sum_{k=0}^{\infty} w_k(z) = \sum_{k=0}^{\infty} \lim_{z \to z_0} w_k(z)
    $$

    **性质 2（逐项积分）**：若级数在区域 $B$ 内的分段光滑曲线 $l$ 上一致收敛，且 $w_k(z)$ 为 $l$ 上的连续函数，则级数可沿 $l$ **逐项积分** ：

    $$
    \int_l \sum_{k=0}^{\infty} w_k(z) \, \mathrm{d}z = \sum_{k=0}^{\infty} \int_l w_k(z) \, \mathrm{d}z
    $$

    **性质 3（逐项求导）**：若 $w_k(z)$ 在闭区域 $\bar{B}$ 内单值解析，且级数在 $\bar{B}$ 内一致收敛，则级数和也是 $\bar{B}$ 内的单值解析函数，可 **逐项求导** ：
...
!!! abstract "定义 4（绝对一致收敛）"
    对于复函数序列 $\{w_k(z)\}$，存在正数列 $\{m_k\}$，使对区域 $B$ 内的一切 $z$，有 $|w_k(z)| \leq m_k$（$k=0,1,2,\ldots$），而正项级数 $\displaystyle\sum_{k=0}^{\infty} m_k$ 收敛，则复函数项级数 **绝对一致收敛** 。
...
!!! abstract "定理 4（阿贝尔定理）"
    如果级数 $\displaystyle\sum_{k=0}^{\infty} a_k (z - z_0)^k$ 在 $z_1$ 点收敛，那么在以 $z_0$ 为圆心、$|z_1 - z_0|$ 为半径的圆 **内部** 绝对收敛，在 $|z - z_0| \leq |z_1 - z_0|$ 的闭圆盘上 **一致收敛** 。

    如果级数在 $z_1$ 点发散，则在 $|z - z_0| > |z_1 - z_0|$ 的圆外 **处处发散**。
...
!!! abstract "定理 5（比值判别法求收敛半径）"
    若 $\displaystyle\lim_{k \to \infty} \left|\frac{a_{k+1}}{a_k}\right| = l$，则收敛半径
    ...
    **推导过程：**
    ...
    - 当 $r < 1$（即 $|z - z_0| < \frac{1}{l} = R$）时，级数 **绝对收敛**
    - 当 $r > 1$（即 $|z - z_0| > \frac{1}{l} = R$）时，级数 **发散**

    因此，当 $|z - z_0| < R$ 时，幂级数满足柯西收敛准则，在圆内绝对收敛；当 $|z - z_0| > R$ 时，由于不满足柯西判据，级数在圆外发散。故收敛半径为 $R$。

!!! abstract "定理 6（根式判别法求收敛半径）"
    若 $\displaystyle\lim_{k \to \infty} \sqrt[k]{|a_k|} = l$，则收敛半径
    ...
    **推导过程：**
    ...
    - 当 $|z - z_0| \cdot l < 1$（即 $|z - z_0| < \frac{1}{l} = R$）时，级数 **绝对收敛**
    - 当 $|z - z_0| \cdot l > 1$（即 $|z - z_0| > \frac{1}{l} = R$）时，级数 **发散**

    因此收敛半径 $R = \frac{1}{l} = \lim_{k \to \infty} \frac{1}{\sqrt[k]{|a_k|}}$。
...
!!! abstract "定理 7（幂级数的解析性）"
    设幂级数 $\displaystyle\sum_{k=0}^{\infty} a_k (z - z_0)^k$ 的收敛半径为 $R$，则
...
!!! abstract "定理 8（泰勒展开定理）"
    设 $f(z)$ 在区域 $B$ 内解析，$z_0$ 为 $B$ 内任一点，$R$ 为 $z_0$ 到 $B$ 边界的最短距离，则当 $|z - z_0| < R$ 时，$f(z)$ 可展开为 **泰勒级数** ：
...
??? note "证明思路"
    ...
    **3. 逐项积分**
    ...
    由于 $\displaystyle\sum_{k=0}^{\infty} \frac{(z - z_0)^k}{(\zeta - z_0)^{k+1}}$ 在 $C_{R_1}$ 上一致收敛，且 $f(\zeta)$ 在 $C_{R_1}$ 上有界，因此：

    $$
    \sum_{k=0}^{\infty} f(\zeta) \frac{(z - z_0)^k}{(\zeta - z_0)^{k+1}} \text{ 在 } B \text{ 内一致收敛} \quad \Rightarrow \quad \text{可逐项积分}
    $$
...
!!! abstract "定义 5（解析延拓）"
    若 $f_1(z)$ 和 $f_2(z)$ 分别在区域 $B_1$、$B_2$ 内解析，且在 $B_1$ 与 $B_2$ 重叠的区域中有 $f_1(z) = f_2(z)$，则称 $f_2(z)$ 为 $f_1(z)$ 在 $B_2$ 中的 **解析延拓** ，$f_1(z)$ 为 $f_2(z)$ 在 $B_1$ 中的 **解析延拓** 。
...
!!! abstract "定理 9（洛朗展开定理）"
    设 $f(z)$ 在环形域 $R_2 < |z - z_0| < R_1$ 内处处解析，$C$ 为圆环域内绕 $z_0$ 的任一正向简单闭曲线，则 $f(z)$ 在圆环域内可展开为 **洛朗级数** ：
...
!!! abstract "定义 6（孤立奇点）"
    若函数 $f(z)$ 在点 $z_0$ 处不解析（或没有定义），但在 $z_0$ 的某个 **空心邻域** $0 < |z - z_0| < R$（$R \leq +\infty$）内解析，则称 $z_0$ 为 $f(z)$ 的 **孤立奇点** 。
...
!!! info "极点的特征"
    若 $z_0$ 是 $f(z)$ 的 $m$ 阶极点，则在 $z_0$ 的去心邻域内
...
!!! warning "本性奇点的极限特点"
    在本性奇点的邻域内，$\displaystyle\lim_{z \to z_0} f(z)$ **不存在且不为** $\infty$ 。

    例如 $\displaystyle\lim_{z \to 0} e^{1/z}$ 不存在（沿正实轴趋于 $+\infty$，沿负实轴趋于 $0$）。

### （三）函数在无穷远点的性态

#### 1. 无穷远点为孤立奇点的定义

!!! abstract "定义 8（无穷远点的孤立奇点）"
    如果函数 $f(z)$ 在无穷远点 $z = \infty$ 的去心邻域 $R < |z| < +\infty$ 内解析，则称点 $\infty$ 为 $f(z)$ 的 **孤立奇点** 。
...
| 概念 | 定义要点 |
|:---:|:---|
| **收敛半径** | 比值法 $R = \displaystyle\lim_{k \to \infty} \left|\frac{a_k}{a_{k+1}}\right|$ 或 根式法 $R = \displaystyle\lim_{k \to \infty} \frac{1}{\sqrt[k]{|a_k|}}$ |
| **泰勒展开** | $f(z) = \displaystyle\sum_{k=0}^{\infty} a_k (z - z_0)^k$，$a_k = \frac{f^{(k)}(z_0)}{k!}$ |
| **洛朗展开** | $f(z) = \displaystyle\sum_{k=-\infty}^{+\infty} a_k (z - z_0)^k$，含负幂项 |
| **可去奇点** | 洛朗级数 **无** 负幂项，$\lim f(z)$ 存在有限 |
| **极点** | 洛朗级数有 **有限** 负幂项，$\lim f(z) = \infty$ |
| **本性奇点** | 洛朗级数有 **无穷** 负幂项，$\lim f(z)$ 不存在 |
