...
??? info "计算公式"
    ...
    z=\frac{1}{w},
    \quad
    \mathrm{d}z=-\frac{1}{w^2}\,\mathrm{d}w.
    $$

    当 $z=Re^{i\theta}$（$\theta:0\to 2\pi$）逆时针绕行时，
    $w=\frac{1}{R}e^{-i\theta}$ 会在半径 $1/R$ 的小圆上 **顺时针** 绕行一周。
    ...
    =
    \oint_{\gamma\ (\text{顺时针})} f\!\left(\frac{1}{w}\right)\left(-\frac{1}{w^2}\right)\mathrm{d}w
    =
    -\oint_{|w|=1/R\ (\text{逆时针})} f\!\left(\frac{1}{w}\right)\left(-\frac{1}{w^2}\right)\mathrm{d}w.
    $$
    ...
    =
    \frac{1}{2\pi i}
    \oint_{|w|=1/R} \left(-\frac{1}{w^2}f\!\left(\frac{1}{w}\right)\right)\mathrm{d}w.
    $$
    ...
    =
    -\frac{1}{w^2}\left(\cdots+c_{-1}w+c_{-2}w^2+\cdots\right)
    =
    \cdots-\frac{c_{-1}}{w}-c_{-2}-\cdots,
    $$
...
??? info "为什么需要条件 4（保证大圆弧积分消失）"
    上半平面围道法取边界为实轴段 $[-R,R]$ 与上半圆弧 $C_R$。由留数定理：

    $$
    \oint_{\Gamma_R} f(z)\,\mathrm{d}z
    =
    \int_{-R}^{R} f(x)\,\mathrm{d}x
    +
    \int_{C_R} f(z)\,\mathrm{d}z
    =
    2\pi i \sum_{\operatorname{Im}(b_j)>0}\operatorname{Res} f(b_j).
    $$
