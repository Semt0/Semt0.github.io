...
??? note "证明"
    ...
    解为 $\theta(n \log n)$。可归纳证明精确上界是 $T(n) \leq 2n \ln n$：
...
归纳证明：假设对 $k < n$ 命题为真，归纳步骤可证 $T(n) \leq cn$。
...
& 4. \quad \quad \textbf{for } i \leftarrow 1 \textbf{ to } n \textbf{ do} \quad \text{// } i \text{ 为待选列号} \\
& 5. \quad \quad \quad \text{检查 } i \text{ 与前面 } k-1 \text{ 个皇后的相容性} \\
& 6. \quad \quad \quad \textbf{if } \text{相容} \textbf{ then 将 } i \text{ 加入 } S \\
& 7. \quad \quad \textbf{if } S \neq \emptyset \textbf{ then} \\
& 8. \quad \quad \quad j \leftarrow \text{Random}(1, |S|) \\
& 9. \quad \quad \quad x_k \leftarrow S[j] \\
& 10. \quad \quad \quad \text{count} \leftarrow \text{count} + 1 \\
& 11. \quad \quad \quad k \leftarrow k + 1 \\
& 12. \quad \quad \textbf{else } k \leftarrow n + 1 \\
& 13. \quad \textbf{return } \text{count}
\end{aligned}
$$
...
$$
k \geq \lceil \log(1/\varepsilon) \rceil
$$
...
& 1. \quad k \leftarrow \lceil \log(1/\varepsilon) \rceil \\
...
& 8. \quad \quad I_p(X(j+1)) \leftarrow (2 \cdot I_p(X(j)) - W_p \cdot x_j + x_{j+m}) \pmod p \\
& 9. \quad \quad j \leftarrow j + 1 \\
& 10. \quad \textbf{return } 0
\end{aligned}
$$
...
**出错条件**：$I_p(X(j)) = I_p(Y)$ 但 $X(j) \neq Y$。

**出错概率**：乘积大小不超过 $(2m)^n$，整除它的素数个数不超过 $\pi(mn)$。选 $M = 2mn^2$，则出错概率不超过 $1/n$。
